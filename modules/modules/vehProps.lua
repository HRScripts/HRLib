-- These vehicle properties functions are taken from https://github.com/overextended/ox_lib !

local isServer <const> = IsDuplicityVersion()
if isServer then
    ---@param vehicle integer
    ---@param props HRLibVehicleProperties
    function HRLib.SetVehicleProperties(vehicle, props)
        Entity(vehicle).state:set('HRLib:SetVehicleProperties', props, true)
    end
else
    local gameBuild <const> = GetGameBuildNumber()

    AddStateBagChangeHandler('HRLib:SetVehicleProperties', '', function(bagName, _, value)
        if not value or not GetEntityFromStateBagName then return end

        while NetworkIsInTutorialSession() do Wait(0) end

        local entity, times = GetEntityFromStateBagName(bagName), 0

        while not DoesEntityExist(entity) do
            if times > 10000 and not DoesEntityExist(entity) then
                return error('Unable to use an unexisting entity in SetVehicleProperties state bag', 2)
            end

            times += 1

            if not DoesEntityExist(entity) then entity = GetEntityFromStateBagName(bagName) end
        end

        HRLib.SetVehicleProperties(entity, value)
        ...
        if NetworkGetEntityOwner(entity) == GetPlayerServerId(PlayerId()) then
            HRLib.SetVehicleProperties(entity, value)
            Entity(entity).state:set('HRLib:SetVehicleProperties', nil, true)
        end
    end)
    ...
    ---@param vehicle integer
    ---@return HRLibVehicleProperties?
    HRLib.GetVehicleProperties = function(vehicle)
    ...
    HRLib.SetVehicleProperties = function(vehicle, props, fixVehicle) ---@diagnostic disable-line: duplicate-set-field
        ...
        return not NetworkGetEntityIsNetworked(vehicle) or NetworkGetEntityOwner(vehicle) == GetPlayerServerId(PlayerId())
    end
end